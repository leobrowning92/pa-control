...
class PAGUI(Frame):
    ...
    def initUI(self):
        ...
        obtn = Button(self, text="New Chip", command=self.newChip)
        obtn.grid(row=1, column=3, sticky=N)

        obtn = Button(self, text="New Device", command=self.newDevice)
        obtn.grid(row=2, column=3, sticky=N)

        hbtn = Button(self, text="Run", command=self.pulldata)
        hbtn.grid(row=5, column=3, padx=5)

        self.v=IntVar()
        radbtn = Radiobutton(self, text='Diode (VF, IF)',variable=self.v,value=1)
        radbtn.grid(row=5, column=1, padx=5,sticky=N+E)

        radbtn = Radiobutton(self, text='FET (VG, VDS, ID, IG)',variable=self.v,value=2)
        radbtn.grid(row=5, column=2, padx=5,sticky=N+E)
    ...
    def pulldata(self):
        if self.v.get() ==1:
            download_pa.download_data(path=self.entry1.get(),filename=self.entry2.get(),values=['VF','IF'])
        elif self.v.get() ==2:
            download_pa.download_data(path=self.entry1.get(),filename=self.entry2.get(),values=['VG', 'VDS', 'ID', 'IG'])
